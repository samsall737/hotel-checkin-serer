package com.nec.facerecognition.config;
...
public class FaceRecognitionConfig {
	...
	@Bean
	public FaceClient faceClient(@Value("${connection.read.timeout.second:60}") final Long timeoutSeconds,
			@Value("${api-integrate-client.base.url}") final String baseUrl) {
		return new Retrofit.Builder()
				.client(new OkHttpClient.Builder().readTimeout(timeoutSeconds, TimeUnit.SECONDS)
						.connectTimeout(timeoutSeconds*4, TimeUnit.SECONDS)
				...
				.addCallAdapterFactory(RxJava2CallAdapterFactory.create()).build().create(FaceClient.class);
	}
